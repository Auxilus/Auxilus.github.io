...
def motorcontrol():
        ...
        fspeed = open("/sdcard/speed","r+w")
        fspeed.write(str(new_speed))
        fspeed.close()
        if new_speed > 50:
                new_speed = 50
        elif new_speed < 25:
                new_speed = 25
        faleft = open("/sdcard/aleft","w+r")
        if accumleft == "":
            faleft.write(str("0"))
        faleft.write(str(accumleft))
        faleft.close()
        faright = open("/sdcard/aright","w+r")
        if accumright == "":
            faright.write(str("0"))
        faright.write(str(accumright))
        faright.close()
        #print('Left: '+ str(accumleft), 'Right: '+ str(accumright)+ ' Speed: '+ str(new_speed),end='\r')
        if accumleft == 0 and accumright == 0:
                ...
                turnratio = float(accumright) / float(accumleft)
                fturn = open("/sdcard/rot","w+r")
                fturn.write(str(turnratio))
                fturn.close()
                #print "Turn Ratio: ", turnratio,"\r"
                if turnratio <= 0.6:
                         #print("\nrotate left")
                         time.sleep(pause)
                elif turnratio >= 2:
...
def dendriteAccumulate(dneuron):
        f = eval(dneuron)
        #print f
        ddata = str( postsynaptic[dneuron][0])
        ddata2 = str(postsynaptic[dneuron][1])
        #os.system("echo -ne '\033[0;31mAcc   '  ")
        #os.system("echo "+" ["+dneuron+"   :   "+ddata+"  "+ddata2+"]")
        f()
...
def fireNeuron(fneuron):
        ...
        if fneuron != "MVULVA":
                f = eval(fneuron)
                #print f
                fdata = str(postsynaptic[fneuron][0])
                fdata2 = str(postsynaptic[fneuron][1])
                os.system("echo -ne '\033[0;32mFire  '  ")
                os.system("echo  "+" ["+fneuron+"   :   "+fdata+"  "+fdata2+"]")
                firef = open("/sdcard/firef","w+r")
                firef.write(fneuron)
                firef.close()
                #print(fneuron++fdata+fdata2,end='\r')
                f()
...
def runconnectome():
        ...
        for ps in postsynaptic:
                if ps[:3] not in muscles and abs(postsynaptic[ps][thisState]) > threshold:
                        fireNeuron(ps)
                        postsynaptic[ps] = [0,0]
        #for k,v in postsynaptic.iteritems():
            #print "{:<8} {:<15} {:<10}".format(postsynaptic,postsynaptic[ps][0],postsynaptic[ps][1])
            #lable, num = v
            #print "{:<15} {:<15} {:<10} \r".format(k,lable,num)
            #sys.stdout.flush()
...
def intouch():
    dendriteAccumulate("PVDL")
    dendriteAccumulate("PVDR")
    dendriteAccumulate("PVCL")
    dendriteAccumulate("PVCR")
    runconnectome()
    time.sleep(0.01)
...
try:
    while True:
        ...
        #if ini == 1:
        #intouch()
        neurostatef = open("/sdcard/lovegame/neurostate","w")
        neurostatef.write("")
        neurostatef.close()
        neurostatef = open("/sdcard/lovegame/neurostate","a")
        for ps in postsynaptic:
            neurostatef.write(str(0)+"\n")
        neurostatef.close()
        neurostatef = open("/sdcard/lovegame/neurostate","w")
        neurostatef.write("0")
        neurostatef.close()
        neurostatef = open("/sdcard/lovegame/neurostate","a")
        for ps in postsynaptic:
            neurostatef.write(str(postsynaptic[ps][thisState])+"\n")
        neurostatef.close()
        #dist = 15
        tempfile = open("/sys/devices/virtual/thermal/thermal_zone0/temp","r")
        temp = int(tempfile.read())
        #print temp
        #if temp >= 44700:
        #   print("TEMP")
        #    dendriteAccumulate("AFDL")
        #    dendriteAccumulate("AFDR")
        #    runconnectome()
        #else:
        #    dendriteAccumulate("PVDL")
        #    dendriteAccumulate("PVDR")
        ...
        #    dendriteAccumulate("PVCR")
        #    runconnectome()
        collf = open("/sdcard/col","r")
        coll = collf.read()
        collf.close()
        #coll = 1
        if (coll) or coll == 1 or coll == "1" or coll == str(1):
            #print("Nose Touch")
            ...
            runconnectome()
            collf = open("/sdcard/col","w")
            collf.write("")
            collf.close()
            ini = 1
            time.sleep(pause)
        else:
            foodf = open("/sdcard/food","r")
            food = foodf.read()
            foodf.close() 
            #if (food):
            if tfood < 2:
        ...
        ##         CO2 sensor       ##
            #dendriteAccumulate("ADFL")
            #dendriteAccumulate("ADFR")
            #dendriteAccumulate("ASEL")
            #dendriteAccumulate("ASER")
            #dendriteAccumulate("BAGL")
            #dendriteAccumulate("BAGR")                                                          
            #runconnectome()
        #dendriteAccumulate("PVDL")
        #dendriteAccumulate("PVDR")
        #dendriteAccumulate("PVCL")
